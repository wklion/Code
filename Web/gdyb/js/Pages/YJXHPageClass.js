...
var YJXHPageClass = PageBase.prototype.extend({
    ...
    renderMenu: function () {
         ...
         */
        if (GDYB.GridProductClass.currentUserDepart == "undefined" || GDYB.GridProductClass.currentUserDepart == null) {
            alertModal("请注意，您尚未登录！");
            return;
        }
        else
            var dc = GDYB.GridProductClass.currentUserDepart.departCode;
		...
		<div class='opt-panel-container opt-panel-yjxh' name='yjxh'>
            ...
            <button value="ljyb_sk" class="active">强天气实况</button>
            <button value="radar_mcr">35dbz回波</button>
            <button value="radar_mvil">液态含水量</button>
        	</div>
				<div style="margin: 5px 0px 0px 0px;">
						<button value="grapes_3km_cr">1H反射率预报</button>
						<button value="ljyb_dq">短强预报</button>
						<button value="ljyb_qs">临近预报</button>
				</div>
    		</div>
			</section>
			<section style="margin-bottom: 10px;">
			...
			<section style="margin-bottom: 10px;">
				...
				<div id="grid-wrapper" style="display:block; width:100%; height:400px; padding:0; margin:2px 0; border: 1px solid #02425e;">
        ...
        var htmlstr_optCard = `
			<div class='menu_change active' name='yjxh'>预警信号</div>
			<div class='menu_change' name='yjcx'>预警查询</div>
		`;
     ...
     */
    optionFilters(sel) {
        let [tmpAny, newResAny, oldResAny] = [[], [], this.oldQueryResult]; // 参与过滤
        let [countRes, notes] = [[], {}]; // 用于统计灾害类型出现的次数
