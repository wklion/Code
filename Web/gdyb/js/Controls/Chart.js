...
//						ctx.stroke();
                ...
                if(config.showValue){
                    ...
                    for (var k=0; k<data.datasets[i].data.length; k++){
                        if(data.labels[k] != "") {
                            ...
                            ctx.fillText(data.datasets[i].data[k], yAxisPosX + (valueHop * k) + ctx.measureText(txt).width / 2, xAxisPosY - 10 - animPc * (calculateOffset(data.datasets[i].data[k], calculatedScale, scaleHop)));
                        }
                    }
                }
			}
		...
		function drawScale(){
			...
			ctx.fillStyle = config.scaleFontColor;
			for (var i=0; i<data.labels.length;i++){
                if(data.labels.length>60&&i%6!=0)//by zoujie
                    continue;
				ctx.save();
				if (rotateLabels > 0){
	...
	var Bar = function(data,config,ctx){
		...
		if (!config.scaleOverride){
			...
			calculatedScale = calculateScale(scaleHeight,valueBounds.maxSteps,valueBounds.minSteps,valueBounds.maxValue,valueBounds.minValue,labelTemplateString,config.Expand);
		}
		else {
			calculatedScale = {
				steps : config.scaleSteps,
		...
		function drawScale(){
			...
			for (var j=0; j<calculatedScale.steps; j++){
                ...
                var deviation = -8;
                if(config.deviationY)
                    deviation += config.deviationY;
				if (config.scaleShowLabels){
					ctx.fillText(calculatedScale.labels[j],yAxisPosX+deviation,xAxisPosY - ((j+1) * scaleHop));
				}
			}
	...
	function calculateScale(drawingHeight,maxSteps,minSteps,maxValue,minValue,labelTemplateString,Expand){
