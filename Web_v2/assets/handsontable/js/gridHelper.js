 ...
 * @exports gridHelper
 * @version 1.1.2
 * @author <rexer @rexerwang>
 *
 * created on 2017-11-13 by webpack
 */
var gridHelper =
...
/* harmony export (immutable) */ __webpack_exports__["paramize"] = paramize;
/* harmony export (immutable) */ __webpack_exports__["initGrid"] = initGrid;
/* harmony export (immutable) */ __webpack_exports__["exportFile"] = exportFile;
/* harmony import */ var __WEBPACK_IMPORTED_MODULE_0_babel_runtime_core_js_array_from__ = __webpack_require__(29);
...
/* harmony import */ var __WEBPACK_IMPORTED_MODULE_3_handsontable__ = __webpack_require__(61);
...
/* harmony import */ var __WEBPACK_IMPORTED_MODULE_4_excellentexport_excellentexport__ = __webpack_require__(62);
/* harmony import */ var __WEBPACK_IMPORTED_MODULE_4_excellentexport_excellentexport___default = __webpack_require__.n(__WEBPACK_IMPORTED_MODULE_4_excellentexport_excellentexport__);
...
var defaults = {
  ...
  sortIndicator: true,
  filters: true,
  dropdownMenu: ['remove_col', SEPARATOR, 'make_read_only', SEPARATOR, 'alignment', SEPARATOR, 'filter_by_condition', 'filter_action_bar', SEPARATOR, 'filter_by_value'],
  contextMenu: true,
  minSpareRows: 0,
...
function paramize(cols) {
  ...
  cols.forEach(function (item) {
    var _Object$assign = __WEBPACK_IMPORTED_MODULE_1_babel_runtime_core_js_object_assign___default()({
      className: 'htCenter htMiddle'
    }, item),
        title = _Object$assign.title,
        col = __WEBPACK_IMPORTED_MODULE_2_babel_runtime_helpers_objectWithoutProperties___default()(_Object$assign, ['title']);
...
function initGrid(container, cols) {
  ...
  var removed = arguments.length > 4 && arguments[4] !== undefined ? arguments[4] : true;

  var $container = container;
  var className = 'grid-wrapper';
  if (typeof container === 'string') {
    $container = window.document.querySelector(container);
  }
  if (!($container instanceof window.Element)) throw new TypeError('Invalid container.');

  if (removed) {
    var $olds = $container.getElementsByClassName(className);
    $olds && $olds.length && __WEBPACK_IMPORTED_MODULE_0_babel_runtime_core_js_array_from___default()($olds).forEach(function (ele) {
      $container.removeChild(ele);
    });
  }

  var $grid = window.document.createElement('div');
  $grid.className = className;
  $container.appendChild($grid);

  var hot = new __WEBPACK_IMPORTED_MODULE_3_handsontable___default.a($grid, paramize(cols, data, opts));

  return hot;
}

function exportFile(instance) {
  var excel = arguments.length > 1 && arguments[1] !== undefined ? arguments[1] : true;
  var filename = arguments.length > 2 && arguments[2] !== undefined ? arguments[2] : 'export';
  var tablename = arguments.length > 3 && arguments[3] !== undefined ? arguments[3] : 'export';
  var filter = arguments[4];

  var header = instance.getColHeader();
  var dataSource = instance.getData();
  var data = isFunction(filter) ? filter(dataSource, header) : dataSource;

  var table = window.document.createElement('table');

  var headerTR = window.document.createElement('tr');
  ...
  for (var row = 0, rsize = data.length; row < rsize; row++) {
    var rowData = data[row];
    var tr = window.document.createElement('tr');
    for (var col = 0, csize = rowData.length; col < csize; col++) {
  ...
  filename += excel ? '.xls' : '.csv';
  var a = window.document.createElement('a');
  if (excel ? __WEBPACK_IMPORTED_MODULE_4_excellentexport_excellentexport___default.a.excel(a, table, tablename) : __WEBPACK_IMPORTED_MODULE_4_excellentexport_excellentexport___default.a.csv(a, table)) {
