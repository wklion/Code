            ...
            if (GDYB.GridProductClass.layerMagic.features.length > 0 && t.arrayTag.length > 0) {
                ...
                for (var i = 0; i < dtGrid.rows; i++) {
                    ...
                    for (var j = 0; j < dtGrid.cols; j++) {
                        ...
                        gridRow.push(dtGrid.getValue(0, j, i));
                        if(gridDirectionOld != null)
                            gridDirectionRow.push(dtGrid.getValue(1, j, i));
                    }
                ...
                for (var i = 0; i < dtGrid.rows; i++) {
                    for (var j = 0; j < dtGrid.cols; j++) {
                        if (arrayTagOld[i][j]) {
                            //var dValue = dtGrid.getValue(0, j, i);
                            var dValue = gridOld[i][j];
                            //dtGrid.setValue(0, j, i, dValueDefault);
                            var ii = i + yOffset;
                            var jj = j + xOffset;
                            if (ii < 0 || ii >= dtGrid.rows || jj < 0 || jj >= dtGrid.cols)
                                continue;
                            dtGrid.setValue(0, jj, ii, dValue);
                            if(gridDirectionOld != null)
                                dtGrid.setValue(1, jj, ii,  gridDirectionOld[i][j]);
...

                                {
                                    for (var k = nIndexLeft; k <= nIndexRight; k++) {
                                            ...
                                            gridOld[i][nIndexLeft - 1] == dtGrid.noDataValue || gridOld[i][nIndexRight + 1] == dtGrid.noDataValue) //左右到边界
                                        {
                                            ...
                                            {
                                                dtGrid.setValue(0, k, i, dValueDefault);
                                                continue;
                                            }
                                            else
                                            {
                                                ...
                                                dtGrid.setValue(0, k, i, Math.round(dValueY*10)/10);
                                                continue;
                                            }
                                        }
                                            ...
                                            gridOld[nIndexTop - 1][j] == dtGrid.noDataValue || gridOld[nIndexBottom + 1][j] == dtGrid.noDataValue) //上下到边界
                                        {
                                            ...
                                            else
                                            {
                                                ...
                                                dtGrid.setValue(0, k, i, Math.round(dValueX*10)/10);
                                                continue;
                                            }
                                        }
                                        ...
                                        var dValueX = gridOld[i][nIndexLeft - 1] * (1 - weightX) + gridOld[i][nIndexRight + 1] * weightX;
                                        var dValueY = gridOld[nIndexTop - 1][k] * (1 - weightY) + gridOld[nIndexBottom + 1][k] * weightY;
                                        dtGrid.setValue(0, k, i, Math.round((dValueX + dValueY) / 2.0 * 10.0) / 10.0);
                                        if (dtGrid.getValue(0, k, i) > 25.0)
                                            t.arrayTag[i][k] = false;
                                    }
                                }
                            }
...
//                GDYB.GridProductClass.addLabel(null, map, null);
                ...
                //更新已修改状态
                GDYB.GridProductClass.dataCache.setDataStatus(GDYB.GridProductClass.currentMakeTime, GDYB.GridProductClass.currentVersion, GDYB.GridProductClass.currentDateTime, GDYB.GridProductClass.currentElement, GDYB.GridProductClass.currentHourSpan, 1, dtGrid);
...
//                    var pt = gridRow[j];
                    ...
                    t.arrayTag[i][j] = true;
                    if( dg.getValue(0, j, i) < minValue)
                        minValue =  dg.getValue(0, j, i);
                }
            }
        ...
        for(var i=0; i<this.arrayTag.length; i++){
            for(var j=0; j<this.arrayTag[0].length; j++){
                if(this.arrayTag[i][j]){
                    var targetValue = GDYB.GridProductClass.datasetGrid.getValue(0, j, i);
                    ...
                    if(bMinIsZero && targetValue < 0)
                        targetValue = 0;
                    GDYB.GridProductClass.datasetGrid.setValue(0, j, i, targetValue);
                }
            }
        }
        //更新已修改状态
        var t = GDYB.GridProductClass;
        t.dataCache.setDataStatus(t.currentMakeTime, t.currentVersion, t.currentDateTime, t.currentElement, t.currentHourSpan, 1, GDYB.GridProductClass.datasetGrid);
        GDYB.GridProductClass.dataStack.push(GDYB.GridProductClass.datasetGrid); //压入堆栈
        GDYB.GridProductClass.updateStationLayer();
    };
