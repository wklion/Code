...
function ZDYBSZPageClass(){
    ...
    this.renderMenu = function(){
        ...
        $("#workspace_div").append("<div id='ZDYBSet' style='left:66px;top: 10px;right:360px;bottom:10px;position: absolute;font-size: 12px;background-color: #FFFFFF;overflow: auto;'></div>");
        ...
        $.ajax({
            ...
            success: function (data) {
                ...
                for (var i = 0; i < list.length; i++) {
                    contentProduct += '<div class="dis_menu_head" flag="true" productId="'+list[i].id+'">' + list[i].name + '<span title="删除" class="zdybProductTypeDelete" style="display: none;">×</span></div>' +
                ...
                $("#zdybPanelTop div.dis_menu_head").hover(function(){
                    $(this).find("span").css("display","");
                },function(){
                    $(this).find("span").css("display","none");
                });

                $(".zdybProductTypeDelete").click(function(){
                    if($(this).parent().next().find(".dis_menu_body_item").length != 1){
                        $(this).parent().attr("flag","false");
                        alert("需要先删除该类型所有时次产品");
                        return;
                    }
                    var obj = this;
                    confirmModal("是否删除",function(){
                        zdybDeleteProductType(obj);
                    });
                });
            }
        });
        ...
        $.ajax({
            ...
            success: function (data) {
                ...
                for (var i = 0; i < data.length; i++) {
                    var contentProduct = '<div class="dis_menu_body_item" href="#" elementId="' + data[i].id + '">' + data[i].name + '<span title="删除" class="zdybProductTimeDelete" style="display: none;">×</span></div>';
                    $("#zdybSetProductAdd"+data[i].type).before(contentProduct);
                }
                if(data.length!=0){
                    t.nowElement = data[0];
                    $($("#zdybPanelTop").find("div.dis_menu_body_item")[0]).css("background-color","rgb(49,202,255)");
                ...
                $("#zdybPanelTop").find("div.dis_menu_body_item").click(function(){
                    ...
                    $("#zdybPanelTop").find("div.dis_menu_body_item").css("background-color","");
                    $(this).css("background-color","rgb(49,202,255)");
                    if($(this).hasClass("zdybAddForecastTimeBT")){
                        t.addProduct = true;
                        $(this).before('<div style="padding-left: 38px;"><input id="zdybAddProductTime" style="width: 180px;height: 24px;margin-top: 1px;" type="text"></div>');
                ...
                $("#zdybPanelTop").find("div.dis_menu_body_item").hover(function(){
                    $(this).find("span").css("display","");
                },function(){
                    $(this).find("span").css("display","none");
                });
    ...
    function initZDYBSetPage(obj){
        ...
        contentHtml +="<div class='zdybSetAreaTime' style='display: "+display+"'>";
        contentHtml +="<span style='margin-left: 24px;'>发报时间：</span>" +
            "<select id='zdybSetMakeTime1' class='zdybSetPublishTime'>";
        for(var i=0;i<24;i++){
            if(obj.id!=-1&&parseInt(obj.makeTime.split(",")[0].substr(0,2))==i){
                contentHtml += "<option selected='true'>"+mosaicTime(i)+"</option>";
            }
            else{
                contentHtml += "<option>"+mosaicTime(i)+"</option>";
        ...
        for(var i=0;i<60;i++){
            if(obj.id!=-1&&parseInt(obj.makeTime.split(",")[0].substr(3,2))==i){
                contentHtml += "<option selected='true'>"+mosaicTime(i)+"</option>";
            }
            else{
                contentHtml += "<option>"+mosaicTime(i)+"</option>";
        ...
        contentHtml +="</select>分";
        contentHtml +="<span style='margin-left: 24px;'>截止时间：</span><select class='zdybSetPublishTime'>";
        for(var i=0;i<24;i++){
        ...
        contentHtml +="<span style='margin-left: 24px;'>发报时间：</span>" +
            "<select class='zdybSetPublishTime'>";
        for(var i=0;i<24;i++){
            if(obj.id!=-1&&parseInt(obj.makeTime.split(",")[1].substr(0,2))==i){
                contentHtml += "<option selected='true'>"+mosaicTime(i)+"</option>";
            }
            else{
                contentHtml += "<option>"+mosaicTime(i)+"</option>";
        ...
        contentHtml +="</select>时" +
            "<select class='zdybSetPublishTime'>";
        for(var i=0;i<60;i++){
            if(obj.id!=-1&&parseInt(obj.makeTime.split(",")[1].substr(3,2))==i){
                contentHtml += "<option selected='true'>"+mosaicTime(i)+"</option>";
            }
            else{
                contentHtml += "<option>"+mosaicTime(i)+"</option>";
        ...
        $.ajax({
            ...
            success: function (data) {
                ...
                for(var i=0;i<elementObj.length;i++){
                    t.elementObjectList[elementObj[i].code] = elementObj[i];
                }
                ...
                $(".zdybAddElement").click(function(){
                    ...
                    content += "<div class='zdybSetElementRow'>" +
                        "<span class='zdybElementTitle' >要素：</span><select class='zdybSetElementSelect zdybSetElementSelect"+addNum+"' addNum='"+addNum+"'>";
                    ...
                    for(var i=0;i<elementObj.length;i++){
                        if($.inArray(elementObj[i].name, elementList)==-1){
                            content += "<option flag='"+elementObj[i].code+"'>"+elementObj[i].name+"</option>";
                            if(firstObject == null){
                                firstObject = elementObj[i]
                            }
                        }
                    }
                    if(firstObject == null){
                    ...
                    var hours = firstObject.hours.split(",");
                    ...
                    for(var i=parseInt($("#zdybSetHourInerval").val());i<=parseInt($("#zdybSetHourTotal").val());){
                        if($.inArray(i.toString(), hours)==-1){
                            content += "<div class='zdybSetElementFalse'>"+i+"</div>";
                        }
                        else{
                    ...
                    $(".zdybSetElementSelect"+addNum).change(function(){
                        var numStr = $(this).attr("addNum");
                        var hours = t.elementObjectList[$(this).find("option:selected").attr("flag")].hours.split(",");
                        var contentDiv = "";
                        for(var i=parseInt($("#zdybSetHourInerval").val());i<=parseInt($("#zdybSetHourTotal").val());){
                            if($.inArray(i.toString(), hours)==-1){
                                contentDiv += "<div class='zdybSetElementFalse'>"+i+"</div>";
                            }
                            else{
                                contentDiv += "<div class='zdybSetElement zdybAddElement"+numStr+"'>"+i+"</div>";
                            }
                            i += parseInt($("#zdybSetHourInerval").val());
                        }
                        contentDiv += "<div class='zdybSetElementDelete zdybAddDelete"+numStr+"' title='删除要素'>×</div></div>";
                        $(this).parent().find("div").remove();
                        $(this).parent().append(contentDiv);
                        $(".zdybAddElement"+numStr).click(function(){
                            zdybSetElementChoice(this);
                        });
                        $(".zdybAddDelete"+numStr).click(function(){
                            var obj = this;
                            confirmModal("是否删除",function(){
                                $(obj).parent().remove();
                            });
        ...
        $.ajax({
            type: 'post',
            url: gridServiceUrl + "services/ForecastfineService/ge tUserStation",
            data: {'para': param},
            dataType: 'text',
            error: function () {
                alert('获取站点错误!');
    ...
    function getZDYBElements(elementObj,elements,hourSpanInerval,hourSpanTotal){
        ...
        var contentHtml = "";
        for (var i = 0; i < elementObj.length; i++) {
            elementNameObj[elementObj[i].code] = elementObj[i];
        }
        var elementList = {};
        ...
        for(var element in elementList){
            var hours = elementNameObj[element].hours.split(",");
            contentHtml += "<div class='zdybSetElementRow'><span class='zdybElementTitle' >要素：</span><span class='zdybElementName' flag='"+element+"'>"+elementNameObj[element].name+"</span><span class='zdybElementTitle' >时效：</span>";
            for(var i=hourSpanInerval;i<=hourSpanTotal;){
                if($.inArray(i.toString(), hours)==-1){
                    contentHtml += "<div class='zdybSetElementFalse'>"+i+"</div>";
                }
                else if($.inArray(i.toString(), elementList[element])!=-1){
    ...
    function zdybSetSave(obj){
        ...
        for(var i=0;i<list.length;i++){
            if($(list[i]).hasClass("zdybSetActive")){
                gdybPublishTime += parseInt($(list[i]).parent().find("select")[5].value).toString()+",";
                endTime += $(list[i]).parent().find("select")[2].value+":"+$(list[i]).parent().find("select")[3].value+":00,";
                makeTime += $(list[i]).parent().find("select")[0].value+":"+$(list[i]).parent().find("select")[1].value+":00,";
            }
            else{
                gdybPublishTime += "-1,";
                endTime += "-1,";
            ...
            gdybType += $($(list[i]).parent().find("select")[4]).find("option:selected").attr("value")+",";
        }
        ...
        makeTime = makeTime.substr(0,makeTime.length-1);
        ...
        if(obj.id!=-1){
            var param = '{"id":'+obj.id+',"publishTime":"'+publishTime+'","gdybPublishTime":"'+gdybPublishTime+'","makeTime":"'+makeTime+'","forecastTime":"'+forecastTime+'","gdybType":"'+gdybType+'","endTime":"'+endTime+'","stationNums":"'+stationNums+'","depart":"'+depart+'","areaCodes":"'+areaCodeStr+'","outType":"'+outType+'","hourSpan":"'+hourSpan+'","hourSpanTotal":"'+hourSpanTotal+'","ui":"'+ui+'"}';
            $.ajax({
                type: 'post',
    ...
    function zdybDeleteProductType(obj){
        $.ajax({
            type: 'post',
            url: gridServiceUrl + "services/ForecastfineService/deleteProductType",
            data: {'para': '{"id":'+parseInt($(obj).parent().attr("productId"))+'}'},
            dataType: 'text',
            error: function () {
                alert('删除失败!');
            },
