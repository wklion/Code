...
var AlertSignalProduce = clazz({
  CONST: { //常量
    step_amount: 2, //总步骤数
    service: WAPDServiceUrl, //预警信号服务
    CTX: CTX,
    icon_url: 'imgs/WarningIcon/', // 图标地址
  ...
  prepare: function() {
    ...
    var userName = GDYB.User.getName();
    var Query = new T.DeferredQuery();
    Query.addQuery(me.CONST.CTX + 'data/gansu.json', null, 'getJSON') //获取区域信息
      .addQuery(me.CONST.CTX + 'produce.html', null, 'get') //获取模版
      // 查询预警模版Context
      ...
      .query(function(areainfo, html, alertContext, forecastors, issuers) {
        if (!areainfo || !html || !T.isPretty(alertContext) ||
        ...
        defer.resolve(doc, areainfo, indexedContext);
      })
      .fail(function(a, b, c) {
