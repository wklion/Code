...
function XTGLPageClass(){
						      ...
						      $("#areaExportForm").ajaxSubmit(function(data){
								  alertModal(data);
						             // 刷新表格
						        		  ...
						        		  $.ajax({
									                type: 'post',
									                url: gridServiceUrl + "services/AreaService/getAreasForGrid",
									                data: {'para': param},
									                dataType:'text',
		...
		// 加载表格数据
			...
			$.ajax({
				...
				success: function (data) {
					...
					mmg.on('cellSelected', function(e1, item1, rowIndex1, colIndex1){
	...
	//	                console.log(colIndex);
						...
						if($(e.target).is('.btn-danger')){
							confirmModal('你确定要删除该行记录吗?',function(){
							...
							var d = dialog({
								title:'修改区域名称',
								content:'区域名称：<input style="height:28px;" id="areaNameUpdate" >', ok:function(){
									// 校验修改内容
									...
									if(!areaNameUpdateValue && typeof(areaNameUpdateValue)!="undefined"){
										...
										$.ajax({
											type: 'post',
											url: gridServiceUrl + "services/AreaService/updateAreaName",
											data: {'para': param},
											dataType:'text',
											error: function () {
												alertModal('修改失败');
											...
											success: function (data) {
												if(data=true){
													alertModal("修改成功");item.name = areaName;
