...
ZNZSPageClas.prototype = {
    ...
    renderMenu() {
      ...
      <div id="znzsPage">
        <div class="topPanel">
         <input type="date" class="currentDate" value="2018-07-04" id="currentDate">
    ...
    initEvent() {
        ...
        let now = new MyDate();
        let time = now;
        $("#currentDate")[0].value = now.format("yyyy-MM-dd");
        if (now.getHours() < 8) {
            param.maketime = new MyDate(time.addHours(-1).setHours(16,0,0)).format("yyyy-MM-dd hh:mm:ss");
            param.datetime = new MyDate(time.addHours(-1).setHours(20,0,0)).format("yyyy-MM-dd hh:mm:ss");

            $("#timeSelect").val("16小时");
        }
        else if (now.getHours() > 8 && now.getHours() < 20) {
            param.maketime = new MyDate(time.setHours(5,0,0)).format("yyyy-MM-dd hh:mm:ss");
            param.datetime = new MyDate(time.addHours(-1).setHours(8,0,0)).format("yyyy-MM-dd hh:mm:ss");

            $("#timeSelect").val("05小时");
        }
        else if (now.getHours() > 20) {
            param.maketime = new MyDate(time.setHours(16,0,0)).format("yyyy-MM-dd hh:mm:ss");
            param.datetime = new MyDate(time.addHours(-1).setHours(20,0,0)).format("yyyy-MM-dd hh:mm:ss");
            $("#timeSelect").val("16小时");
        }
        /*$("#currentDate")[0].value = time.format("yyyy-MM-dd");
        param.datetime = time.addHours(3).format("yyyy-MM-dd hh:mm:ss");*/
    ...
    getDataFormPOS() {
        ...
        var url = gridServiceUrl + "GridService/grid2points"; //格点转任意点
