...
RadarMenu.prototype = {
  ...
  initEvent: function () {
    ...
    $("#puzzle").find("button").click(function () {
      ...
      $("#puzzle button").removeClass("active");
      ...
      var name = $(this).text();
      if (id == "pup") {
        me.getImgProduct(eleID,name);
      }
      else if (id == "swan") {
        me.getImgProduct(eleID,name);
      }
      else if (id == "cappiDiv") {
        var cappiP = $(this).text();
        me.getImgProduct("cappi",name,cappiP);
      }
      $("#station button").removeClass("active");
   ...
   */
  getImgProduct: async function (id, name,cappiP) {
    var me = this;
    var com = new Common();
    var result = await com.getRadarImgLastDate(id, cappiP);
    if (result.isFind) {
      me.displayImgProduct(result.url, result.bounds);
      $("#live_date").html(result.dateTime);
      GDYB.Title.add(name,result.dateTime,true);
    }
    else {
