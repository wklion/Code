...
GridProductClass.prototype.currentPost = null; //当前岗位（关键岗：前3天0-72，值班岗：后四天72-240，全部：0-240，首席岗：0-240）
...
GridProductClass.prototype.layerFillRangeColor = null; //格点图层
...
GridProductClass.prototype.addValue = 0.1;  //加一
GridProductClass.prototype.subValue = -0.1; //减一
    ...
    * */
    GridProductClass.prototype.callModel = function(type, maketime) {
        ...
        this.getGrid(function(datasetGrid){
            if(datasetGrid != null && datasetGrid.rows > 0)
            {
...
GridProductClass.prototype.addGrid = function(recall, pMap, strCallModel){
    ...
    this.getGrid(function(datasetGrid){
        ...
        if(datasetGrid == null){
            //alert(bCallModel?"调入失败，无对应时效数据":"无数据");
            alertFuc(bCallModel?"调入失败，无对应时效数据":"无数据");
        }
        else if(t.layerLabel.visibility && (element != "10uv" && element != "wmax"))
        {
...
GridProductClass.prototype.getGrid = function(recall, element, type, level, hourspan, maketime, version, datetime, bCallModel){
    ...
    function getGridDetail(){
        var url= bCallModel? gridServiceUrl+"services/GridService/callModel" : gridServiceUrl+"services/GridService/getGrid";
        $.ajax({
...
//                }
            ...
            try {
                if (data != null && typeof(elements) != "undefined") {
                    ...
                    /*for (var i = 0; i < data.length; i++) {
                        var gridInfo = data[i];
                        t.dataCache.getData(maketime, version, datetime, gridInfo.element, gridInfo.hourSpan,function(dataCache){
                            if (dataCache != null && dataCache.status == 0) {
                                var status = 0;
                                if (typeof(gridInfo.userName) != "undefined" && gridInfo.userName != "") //已提交
                                {
 ...
 * 返回：是否成功
 * */
GridProductClass.prototype.ridGridBeyondBounds = function(dg, gr){
    var result = false;
    if (dg == null || gr == null)
        return result;
    ...
    for(var i = 0; i<= dg.rows-1; i++)
    {
        ...
        for(var j = 0; j<=dg.cols-1; j++)
        {
            if(i<nTop || j<nLeft || i>nBottom || j>nRight){
                dg.setValue(0, j, i, dg.noDataValue);
                continue;
            }
            if((j+0.5) < arrayXSort[0]){
                dg.setValue(0, j, i, dg.noDataValue);
            ...
            if((j-0.5) > arrayXSort[arrayXSort.length-1]){
                dg.setValue(0, j, i, dg.noDataValue);
                continue;
            }
            var num = 1;
            while (num < arrayXSort.length){
                if(num + 1 <arrayXSort.length){
                    if((j+0.5) < arrayXSort[num+1] && (j-0.5) > arrayXSort[num]){
                        dg.setValue(0, j, i, dg.noDataValue);
                    }
                }
                num += 2;
...
GridProductClass.prototype.getExtremebyRegion = function(dg, gr){
    ...
    var minValue = dg.dMax;
    var maxValue = dg.dMin;
    var minValueX = 0;
    var minValueY = 0;
    var maxValueX = 0;
    var maxValueY = 0;
...
GridProductClass.prototype.saveGridProduct = function(type, userName, forecaster, issuer){
    ...
    if(Number(currentHourSpan) <= hourSpanRange.min || Number(currentHourSpan) > hourSpanRange.max){
        alertModal(currentHourSpan + "小时，不是您的预报的范围。");
        return;
    }
    ...
    if(t.currentElement == "10uv" || t.currentElement == "wmax")
    {
        for(var i=0; i< t.datasetGrid.rows; i++)
        {
            for(var j=0; j< t.datasetGrid.cols; j++)
            {
                var dir = t.datasetGrid.getValue(1, j, i);
                var spd = t.datasetGrid.getValue(0, j, i);
    ...
    else
    {
        ...
        for(var i=0; i< t.datasetGrid.rows; i++)
        {
            for(var j=0; j< t.datasetGrid.cols; j++)
            {
                values.push(t.datasetGrid.getValue(0, j, i));
                if(hasTag)
    ...
    var url=gridServiceUrl+"services/GridService/saveGrid";
    $.ajax({
...
GridProductClass.prototype.saveGridProducts = function(recall, type, userName, forecaster, issuer, element, hourSpans){
    ...
    function getEachHourSpanData(hourSpan){
        if(Number(hourSpan) > hourSpanRange.min && Number(hourSpan) <= hourSpanRange.max){
            t.dataCache.getData(t.currentMakeTime, t.currentVersion, t.currentDateTime, element, hourSpan,function(dataCache){
                    ...
                    for(var i=0; i< datasetGrid.rows; i++)
                    {
                        for(var j=0; j< datasetGrid.cols; j++)
                        {
                            ...
                            if(isNaN(dir) || isNaN(spd)){
                                ...
                                alertModal(t.getElementNameByElement(element)+"时效"+hourSpan+"：格点值存在非法字符，无法提交。");
                                $("#div_progress").css("display", "none");
                                return;
                            }
                            var tmp = (270.0-dir) * Math.PI /180.0;
                ...
                if(element == "10uv"){
                    gridinfosData[hourSpan] = "{id:" + id + ", departCode:'"+t.currentUserDepart.departCode+"',element:'"+ element + "',type:'"+ type + "',level:'"+ t.currentLevel
    ...
    var i=0;
    var endNum = 20;
    function createGridInfos(){
        if(element == "10uv"){
            gridinfos = "";
            while(i<endNum){
                gridinfos += gridinfosData[hourSpans[i]]+",";
                i++;
                if(i>=hourSpans.length)
                    break;
            }
        }
    ...
    function saveAllHourSpanDatas(){
        ...
        $.ajax({
            ...
            success:function(data){
                if(data)
                {
                    var startNum = (i-20)>0?(i-20):0;
                    var falseHourSpans = "";
                    for(var key in hourSpans) {
                        var hourSpan = hourSpans[key];
                        if(Number(hourSpan) <= hourSpanRange.min || Number(hourSpan) > hourSpanRange.max)
                            continue;
                        if(element == "10uv" && (Number(hourSpan) <= hourSpans[startNum] || Number(hourSpan) > hourSpans[i-1])){
                            continue;
                        }
                        var id = data[key];
                        if(id < 0)
                    ...
                    if(i<hourSpans.length && element == "10uv"){
                        endNum += 20;
                        createGridInfos();
                    }
                    else{
...
//提交（一个格点）产品通用方法，内部和外部均可使用。
GridProductClass.prototype.saveGridProductCommon = function(recall,datasetGrid,id,departCode,type,element,forecastTime,hourSpan,totalHourSpan,level,version,nwpModel,nwpModelTime,userName,forecaster,issuer,makeTime,subjective,remark){
    if(datasetGrid == null)
        return;
    ...
    for(var i=0; i< datasetGrid.rows; i++)
    {
        for(var j=0; j< datasetGrid.cols; j++)
        {
            values.push(datasetGrid.getValue(0, j, i));
            if(hasTag)
                values.push(datasetGrid.tag[i][j]);
        }
    }

    $("#div_progress_title").html("正在提交...");
    $("#div_progress").css("display", "block");

    var url=gridServiceUrl+"services/GridService/saveGrid";
    $.ajax({
        data:{"para":"{id:" + id + ",departCode:'"+departCode+"',element:'"+ element + "',type:'"+ type + "',level:'"+ level
            + "',hourspan:"+ hourSpan + ",hourspanTotal:"+ totalHourSpan + ",maketime:'"+ makeTime + "',version:'"+ version + "',datetime:'"+ forecastTime + "',fromModel:'" + nwpModel + "',fromModelTime:'" + nwpModelTime
            + "',userName:'" + userName + "',forecaster:'" + forecaster + "',issuer:'" + issuer + "',subjective:" + subjective+ ",remark:'" + remark + "',cols:" + datasetGrid.cols
            + ",rows:" + datasetGrid.rows + ",left:" + datasetGrid.left + ",bottom:" + datasetGrid.bottom
            + ",width:" + datasetGrid.width+ ",height:" + datasetGrid.height + ",noDataValue:" + datasetGrid.noDataValue + ",values:'" + values + "'}"},
        url:url,
        dataType:"json",
...
GridProductClass.prototype.exportToMicaps = function(recall, type, strElements, makeTime){
    ...
    var url=gridServiceUrl+"services/GridService/exportToMicaps";
...
GridProductClass.prototype.getGrids = function(recalls, element, elementName, type, level, hourspans, maketime, version, datetime, strCallModel){
    ...
    var url= bCallModel?gridServiceUrl+"services/GridService/callModels" : gridServiceUrl+"services/GridService/getGrids";
    $.ajax({
        ...
        success:function(datas){
            try
            {
                ...
                if(isAllNull){
                    if(bCallModel)
                        alertModal("模式调入失败："+elementName+"无数据");
                }
                else {
                    var bWind = (element == "10uv" || element == "wmax"); //是否为风场，风场具有两个字段（风向、风速），在dvalues中交叉表示
                    var dMin = 9999;
                    ...
                    for (var k = 0; k < datas.length; k++) {
                        ...
                        if (dvalues != null && dvalues.length > 0) {
                            ...
                            for (var i = 0; i < data.rows; i++) {
                                var tagLine = [];
                                nIndexLine = data.cols * i * dimensions;
                                for (var j = 0; j < data.cols; j++) {
                                    nIndex = nIndexLine + j * dimensions;
                                    if (bWind) {
                                        z = dvalues[nIndex + 1];
                                        grid.push(Math.round(dvalues[nIndex+1])); //风速在前
                                        grid.push(Math.round(dvalues[nIndex]));   //风向在后
...
GridProductClass.prototype.showStation = function(recall, stationType, height){
    var t = this;
    if(t.stations == null){
        var areaCode = t.currentUserArea;
...
GridProductClass.prototype.showFocusAreaByType = function(recall){
    ...
    var t = this;
    var type =  t.areaType;
    var url=gridServiceUrl+"services/AreaService/getAreasByType";
    $.ajax({
        data:{"para":"{type:'"+ type + "'}"},
        url:url,
        dataType:"json",
...
GridProductClass.prototype.getFillColorItems = function(element){
    ...
    if(element == "2t")
        ...
        items = heatMap_GDYB_PPH;
    else if(element == "fog")
        items = heatMap_GDYB_FOG;
    else if(element == "hz")
        items = heatMap_GDYB_HZ;
    else if(element == "sand")
        items = heatMap_GDYB_SAND;
    else if(element == "rat")
        items = heatMap_GDYB_RAT;
    else if(element == "hail")
        items = heatMap_GDYB_HAIL;
    else if(element == "ssm")
        items = heatMap_GDYB_SSM;
    else if(element == "smg")
        items = heatMap_GDYB_SMG;
    else
        items = heatMap_TempStyles;
    return items;
};
 ...
 * 返回：是否包含
 * */
GridProductClass.prototype.contain = function(dg, gr, dvalue,condition){
    var result = false;
    if (dg == null || gr == null)
        return result;
